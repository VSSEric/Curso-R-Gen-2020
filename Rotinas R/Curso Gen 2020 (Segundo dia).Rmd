...
```{r}
setwd("C:\\Users\\Usuario\\Desktop\\CURSO R GEN 2020")
dir()
```
...
```{r}
AOVdados2<-aov(resp ~ trat, data=dados2)
```
...
```{r}
car::dwt(lm(resp ~ trat, data=dados2))
```
...
#Raiz Quadrada
```{r}
dados2$RQUAD<-dados2$resp^(1/2)
```

#Raiz Cubica
```{r}
dados2$RCUB<-dados2$resp^(1/3)
```

#Log
```{r}
dados2$LOG<-log(dados2$resp)

```

#Potência
```{r}
dados2$POT2<-dados2$resp^2
```
